...
const API = (name, path = '/', query = {}, apikeyqueryname) => (name in etc.APIs ? etc.APIs[name] : name) + path + (query || apikeyqueryname ? '?' + new URLSearchParams(Object.entries({ ...query, ...(apikeyqueryname ? { [apikeyqueryname]: etc.APIKeys[name in etc.APIs ? etc.APIs[name] : name] } : {}) })) : '')

//const opts = new Object(yargs(process.argv.slice(2)).exitProcess(false).parse())
const prefix = new RegExp('^[' + (etc.prefix || '‎xzXZ/i!#$%+£¢€¥^°=¶∆×÷π√✓©®:;?&.\\-').replace(/[|\\{}()[\]^$+*?.\-\^]/g, '\\$&') + ']')
...
const isOwner = (m) => {
    const isROwner = (etc.owner + "@c.us").includes(m.author)
...
const isAdmin = async (m) => {
    ...
    const botChatObj = groupChat.participants.find(chatObj => chatObj.id.user === m.author)?.isAdmin;
    return botChatObj || isOwner(m)
}

const isBotAdmin = async (m) => {
    const groupChat = await m.getChat();
    const botChatObj = groupChat.participants.find(chatObj => chatObj.id.user === conn.info.wid.user)?.isAdmin;
    return botChatObj
}
