...
const dfail = (type, m) => {
  let msg = {
    ...
    limit: "insufficient Limits",
    level: "insufficient Levels",
...
const isOwner = (m) => {
  const isROwner = (etc.owner + "@c.us").includes(m.sender || m.from);
  return isROwner || m.fromMe;
};

const isPrems = (m) => {
  const isPremium = db.data.users[m.sender].premium
  return isPremium || isOwner(m);
}
...
const isAdmin = async (m) => {
  ...
  const authorIsAdmin = groupChat.participants.find(
    (chatObj) => chatObj.id._serialized === m.sender
  )?.isAdmin;
