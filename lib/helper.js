...
const isAdmin = async (m) => {
    ...
    const authorIsAdmin = groupChat.participants.find(chatObj => chatObj.id._serialized === m.author)?.isAdmin;
    return authorIsAdmin
}

const isBotAdmin = async (m) => {
    const groupChat = await m.getChat();
    const botIsAdmin = groupChat.participants.find(chatObj => chatObj.id.user === conn.info.wid.user)?.isAdmin;
    return botIsAdmin
}
