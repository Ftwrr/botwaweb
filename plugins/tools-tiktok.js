...
let handler = async (m, { args, client, usedPrefix, command }) => {
    ...
    const url = nowm || wm
    //m.reply(url)
    /*if (!url.includes(music)) await */ m.reply( await MessageMedia.fromUrl(url, { unsafeMime: true }))//sock.sendFile(m.chat, url, 'tiktok.mp4', `*${nickname}*\n@${unique_id}\n\n${desc ? desc : ''}`.trim(), m)
    //sock.sendFile(m.chat, music, 'tiktok.mp3', title, m)
}
